...
cInverter::cInverter(std::string devicename, int qpiri, int qpiws, int qmod, int qpigs) {
    ...
    if (!(devicename.compare("/dev/hidraw0")))
    {
        struct stat buffer;
        int result = stat("/dev/hidraw0", &buffer);
        if (result != 0) {
	    lprintf("INVERTER: Unable to find device file %s ", devicename.c_str());
            result = stat("/dev/hidraw1", &buffer);
            if (result == 0)
            {
		lprintf("INVERTER: Found /dev/hidraw1");
                device = "/dev/hidraw1";
            }
            else
            {
                lprintf("INVERTER: Unable to find device file /dev/hidraw1");
                result = stat("/dev/hidraw2", &buffer);
                if (result == 0)
                {
                    device = "/dev/hidraw2";
                }
                ...
                {
                    lprintf("INVERTER: Unable to find device file 0/1/2!! reverts to 0.");
                }
            }
        }
    }
...
bool cInverter::query(const char *cmd, int replysize) {
    ...
    if (fd == -1) {
        lprintf("INVERTER: Unable to open device file %s (errno=%d %s)",this->device.data(), errno, strerror(errno));
        sleep(5);
        return false;
    }
